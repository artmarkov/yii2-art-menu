...
/* @var $searchLinkModel artsoft\menu\models\search\SearchMenuLink */
...
<?php if (empty($queryParams)): ?>
    <h4>
        <?= Yii::t('art/menu', 'Please, select menu to view menu links...') ?>
    </h4>
<?php elseif (!isset($parentId) && $dataProvider->count == 0): ?>
    <h4>
        <?= Yii::t('art/menu', 'Selected menu doesn\'t contain any link. Click "Add New Link" to create a link for this menu.') ?>
    </h4>
<?php else: ?>
    <?=
    ListView::widget([
